...
본 시스템은 트럭, 설비, 관제 서버, GUI를 통합한 시연 가능한 수준의 IoT 제어 시스템으로 완성되었으며, 구조적으로 다양한 확장 가능성을 내포하고 있습니다.  
다음은 구현 시 나타난 주요 제약과 이에 대한 구체적인 개선 가능성을 함께 정리한 내용입니다.


### 🧩 다중 트럭 운용 제한

- 📌 **현재**: 시스템은 1대 트럭 기준 FSM 및 GUI 흐름으로 구성되어 있으며, GUI 시각화와 미션 큐가 단일 트럭에 고정되어 있음.
- 🔧 **개선 방향**: `fsm.contexts[truck_id]`, `TruckFSMManager`, `TruckStatusManager` 구조는 이미 다중 트럭 운용이 가능하도록 설계됨.  
  → GUI와 큐 구조만 확장하면 다중 FSM 기반의 **병렬 트럭 운용 시뮬레이터**로 고도화 가능.



### 🔋 배터리 가상값 기반 운영

- 📌 **현재**: 트럭 배터리 상태는 시뮬레이션 값으로 처리되며, 실제 센서(INA226 등)는 미연동.
- 🔧 **개선 방향**: 전류/전압 실시간 측정 센서 도입 시, SoC 예측 및 충전 제어 로직과 연계 가능.  
  → 향후 **경로 기반 에너지 최적화**, **스마트 충전소 제어** 등으로 발전 가능.



### 📡 설비 제어 단방향 통신 구조

- 📌 **현재**: 서버 → 설비로의 단방향 명령 송신 후, 설비 → 서버로 상태 회신만 존재하며, 핸드셰이크 구조 미비.
- 🔧 **개선 방향**: 설비 ACK 응답을 기반으로 한 재전송/재시도 구조 도입 시 **에러 대응 및 복구 시스템** 구축 가능.  
  → 예: 상태 응답 누락 시 N초 후 재시도, 실패 누적 시 이벤트 로그 자동 기록 등.



### 🔄 설정 저장 기능 미구현

- 📌 **현재**: GUI 내 통신 설정, 장치 등록 등은 세션 내 임시 저장이며, 프로그램 재시작 시 초기화됨.
- 🔧 **개선 방향**: 설정 내용을 JSON 또는 SQLite 등으로 저장하고 GUI 실행 시 자동 로드하는 구조 도입 시,  
  사용자 편의성 및 **운영 연속성** 향상 기대.



### 🧠 FSM 디버깅 지원 부족

- 📌 **현재**: FSM 상태 변화 및 이벤트 흐름은 내부 로그(`truck_fsm.log`)로만 기록되며, 시각화되지 않음.
- 🔧 **개선 방향**: 상태 전이 트리거를 기반으로 FSM 다이어그램 자동 생성 및 GUI 연동 시,  
  실시간 FSM 상태 시각화/분석이 가능해져 **디버깅 및 교육 활용성** 증가.


> 📌 현재 한계는 시간과 자원의 제약 속에서 필요한 기능을 우선 구현한 결과이며, 시스템 구조는 확장 가능성을 충분히 반영하고 있습니다.
