...
D.U.S.T. (Dynamic Unified Smart Transport) 는 RFID 기반 위치 인식을 통해 경로를 따라 주행하는 소형 AGV(Automated Guided Vehicle) 를 중심으로, 게이트, 컨베이어 벨트, 적재소 등 물류 설비와 함께 실시간으로 통합 제어하는 IoT 기반 운송 관제 시스템입니다.
...
단순 주행 제어나 센서 통신 구현 수준을 넘어서, **AGV 이동 → 시설 제어 → 상태 모니터링 → 사용자 인터페이스**까지 **하나의 FSM 기반 제어 흐름으로 통합 제어**하는 것을 핵심 목표로 설정하였습니다.
...
- **중앙 제어 서버가 트럭과 설비를 실시간으로 판단 및 제어**
...
### 🎯 주요 기술적 지향점
- FSM 기반 제어 흐름 설계 → 각 구성 요소의 상태를 명확히 정의하고, 상태 전이에 따라 명령/응답을 일관되게 수행
- 이기종 통신 구조 통합 → TCP(트럭), Serial(설비), HTTP GUI 통신을 단일 FSM 흐름 안에서 처리
- 트럭 및 설비 간 실시간 상호작용 구현 → 설비 응답 기반으로 다음 상태로 자동 전이되는 제어 흐름 구현
- 모듈화된 코드 구조 → 트럭, 설비, 미션, 상태 기록 등 기능별 모듈 분리로 유지보수성과 확장성 강화
- 시뮬레이션 및 테스트 유연성 확보 → FakeSerial 등 가상 환경 지원을 통해 실제 하드웨어 없이도 테스트 가능
...
| 기능            | 설명                                                                                                                                         |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **메인 모니터링 탭** | 전체 맵에서 트럭 위치 및 진행 상황을 실시간으로 시각화합니다. 사용자는 AGV의 상태(FSM 상태, 위치, 미션 진행 상황 등)를 한눈에 확인할 수 있으며, 트럭의 동작 흐름을 직접 관제할 수 있습니다.                         |
| **미션 관리 탭**   | AGV가 수행할 미션을 수동으로 등록하거나 삭제할 수 있으며, 현재 진행 중인 미션의 상세 정보도 확인 가능합니다. 이는 트럭 FSM 흐름에 직접 영향을 주는 기능으로, 미션 생성 → 배정 → 완료까지의 제어 흐름을 GUI에서 제어할 수 있습니다. |
| **이벤트 로그 탭**  | 트럭과 설비의 상태 변화, 명령 수행, 센서 감지, 오류 발생 등의 주요 이벤트를 실시간으로 확인할 수 있어, AGV 운영 상태를 정밀하게 추적할 수 있습니다.                                                  |
| **Setting 탭** | 트럭 ID, 설비 포트, 통신 설정 등을 구성하여 AGV 및 제어 서버 간 통신 환경을 설정합니다. 해당 설정은 실제 운행에 적용되는 구성으로, 시스템 시작 시 필수로 지정해야 하는 항목들입니다.                              |
