...
- **중앙 제어 서버가 AGV과 설비를 실시간으로 판단 및 제어**
...
| SR_05 | 중앙 통제 기능 | AGV과 시설을 통합 제어하는 FSM 기반 중앙 서버 |
...
- AGV과 1:N 관계를 가지며, 미션 진행 단계를 시간 순으로 관리합니다.
...
| 기능 | 설명 |
|------|------|
| **FSM 기반 제어 흐름** | AGV과 시설의 상태를 FSM으로 관리하며, 상태에 따라 명령을 자동 전송합니다. |
| **TCP / Serial 통신 처리** | AGV과는 TCP로, 시설과는 Serial로 통신하며 양방향 명령/상태 처리를 수행합니다. |
...
| **이벤트 로그 탭**  | AGV과 설비의 상태 변화, 명령 수행, 센서 감지, 오류 발생 등의 주요 이벤트를 실시간으로 확인할 수 있어, AGV 운영 상태를 정밀하게 추적할 수 있습니다.                                                  |
